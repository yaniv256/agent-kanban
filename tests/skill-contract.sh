...
assert_contains "$skill" 'Human-review handoffs do not belong in Next'
assert_contains "$skill" 'Move the topmost such card to In Progress, ask for'
assert_contains "$skill" 'pause the goal until the human responds.'
...
assert_contains "$skill" 'Use Deep for security, privacy, data-loss, or production incidents.'
assert_contains "$skill" 'Use Standard/Full for repeated failures; cross-component or cross-repository work; likely overlap or conflict with existing solution documents; or an explicit request for Full.'
assert_contains "$skill" 'When more than one trigger applies, the deepest required level wins.'
...
assert_contains "$methodology" 'Checked analysis, a documented failure, an accepted limitation, or a follow-up card does not substitute for remediation.'
assert_contains "$methodology" 'A legitimate no-learning result is a successful closure outcome:'
assert_contains "$methodology" 'A CE Compound execution failure is a bug, not a no-learning result.'
assert_contains "$methodology" 'If agent-runnable work appears while a Human required card is awaiting a response'
assert_contains "$skill" '**Ready for Review** with the review packet.'
assert_contains "$skill" 'Otherwise move it to **Done** only when both'
assert_contains "$skill" 'only human review, approval, or a decision remains.'
...
assert_contains "$repo_root/tests/forward-eval-scenarios.md" 'Scenario 8 — completed review packet does not stall execution'
assert_contains "$repo_root/tests/forward-eval-scenarios.md" 'Scenario 9 — reviewer requests changes'
assert_contains "$repo_root/tests/forward-eval-scenarios.md" 'Scenario 10 — distinguish review from a non-review dependency'
